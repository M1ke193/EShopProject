...
import IconAwesome from "src/components/base/iconawesome";
      ...
      <div className={style.slideWrap}>
        <div className={style.slide} ref={slideElementRef}>
          {products.map((item, index) => (
            <Fragment key={index}>
              <div
                className={`${activeBtn === index ? style.bigImgWarp : style.smallImgWarp
                  } 
                  ${index >= activeBtn + 2 && style.visible}`}
              >
                <img key={index + item.id} src={item.image} />
                <div
                  className={`${style.saleTitle} ${activeBtn === index ? style.visibleTitle : ""
                    }`}
                >
                  <span>From</span>
                  <span>{`$${item.price}${item.price / 10 < 10 ? ".00" : ""
                    }`}</span>
                </div>
              </div>
              {index === products.length - 1 && (
                <div
                  className={`${style.smallImgWarp} ${activeBtn < products.length - 1 ? style.visible : ""
                    }`}
                >
        ...
        {products.map((item, index) => (
          <span key={item.id}>
            <button
              ...
              className={`${style.button} ${activeBtn === index ? style.active : ""
                }`}
            ></button>
          </span>
        ))}
